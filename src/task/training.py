...
class ClassifierTrainingTask(CodeListTask):
    ...
    def process(self):
        ...
        decisions = self.convert_classes_to_original_names(decisions, codelist)
        ...
        train(
            decisions,
            codelist,
            concept_scheme_uri=codelist.concept_scheme_uri,
        )
    ...
    #-----------------------------------------------
    # Methods for saving and managing training datasets (debugging)
    #-----------------------------------------------
    ...
    #-----------------------------------------------
    # Helper methods for processing training data
    #-----------------------------------------------
    ...
    #------------------------------
    # Functions for fetching actions and expression and their associated classes
    #------------------------------
    ...
    def fetch_actions_with_classes(self, include_siblings: bool = False) -> list[dict]:
        ...
        for binding in bindings:
            ...
            if class_uri != no_match_uri and class_uri not in sample["classes"]:
    ...
    def fetch_decisions_with_classes(self) -> list[dict]:
        """Fetch annotated expressions and any human-validation votes.

        The expression may be an action, a plan, or any other ``eli:Expression``;
        """
        concept_scheme_uri = sparql_escape_uri(self.fetch_codelist_uri_for_task())
        ai_graph = sparql_escape_uri(GRAPHS["ai"])
        public_graph = sparql_escape_uri(
            GRAPHS.get("public", "http://mu.semte.ch/graphs/public")
        )
        human_validation_graph = sparql_escape_uri(
            GRAPHS.get(
                "human_validation",
        ...
        expression_filter = self.get_expressions_in_task_filter("?expression")
        member_block = self.member_content_sparql_block("?expression")

        q = Template(
            get_prefixes_for_query(
                "rdf", "eli", "eli-dl", "oa", "epvoc", "dct", "skos", "schema"
            )
            + """
            SELECT ?expression ?body ?voteLabel
                   ?title ?description ?decision_basis ?content ?title_code ?work_type
            ...
            WHERE {
                ...
                GRAPH ?dataGraph {
                    $expression_filter
                    ?expression rdf:type eli:Expression .
                    OPTIONAL { ?expression eli:title ?title }
                    OPTIONAL { ?expression eli:description ?description }
                    OPTIONAL { ?expression eli-dl:decision_basis ?decision_basis }
                    OPTIONAL { ?expression epvoc:expressionContent ?content }
                    $member_block
                }
            }
            GROUP BY ?expression ?body ?voteLabel
                     ?title ?description ?decision_basis ?content
                     ?title_code ?work_type
            """
        ).substitute(
            expression_filter=expression_filter,
            ai_graph=ai_graph,
            public_graph=public_graph,
            human_validation_graph=human_validation_graph,
        ...
        samples_by_expression: dict[str, dict] = {}
        for binding in bindings:
            expression_uri = binding["expression"]["value"]
            sample = samples_by_expression.setdefault(
                expression_uri,
                {
                    "decision": expression_uri,
                    "classes": [],
                    "label_votes": {},
                    "text": self.assemble_expression_text(binding),
                },
            )
        ...
        return list(samples_by_expression.values())